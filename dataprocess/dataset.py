...
class UniRelDataset(Dataset):
    def __init__(
        ...
        self.pred_inputs = tokenizer.encode_plus(self.pred_str, add_special_tokens=False)
    ...
    def __getitem__(self, idx):
        ...
        return {
            "input_ids": torch.tensor(np.array(input_ids, dtype=np.int64), dtype=torch.long),
            "attention_mask": torch.tensor(np.array(attention_mask, dtype=np.int64), dtype=torch.long),
            "token_type_ids": torch.tensor(np.array(token_type_ids, dtype=np.int64), dtype=torch.long),
