...
class UniRelDataProcessor(object):
    def __init__(
        ...
        self.label_map_cache_path = os.path.join(self.task_data_dir, dataset_name + ".dict")
    ...
    def get_train_sample(self, token_len=100, data_nums=-1):
        return self._pre_process(self.train_path, token_len=token_len, is_predict=False, data_nums=data_nums)

    def get_dev_sample(self, token_len=150, data_nums=-1):
        return self._pre_process(self.dev_path, token_len=token_len, is_predict=True, data_nums=data_nums)

    def get_test_sample(self, token_len=150, data_nums=-1):
        samples = self._pre_process(self.test_path, token_len=token_len, is_predict=True, data_nums=data_nums)
    ...
    def get_specific_test_sample(self, data_path, token_len=150, data_nums=-1):
        return self._pre_process(data_path, token_len=token_len, is_predict=True, data_nums=data_nums)
    ...
    def _pre_process(self, path, token_len, is_predict, data_nums):
        ...
        for line in tqdm(data):
            ...
            spo_span_list = set()
            # [CLS] texts [SEP] rels
            head_matrix = np.zeros([token_len + 2 + self.num_rels, token_len + 2 + self.num_rels])
            tail_matrix = np.zeros([token_len + 2 + self.num_rels, token_len + 2 + self.num_rels])
            span_matrix = np.zeros([token_len + 2 + self.num_rels, token_len + 2 + self.num_rels])
            ...
            for spo in line["relation_list"]:
                ...
                spo_tail_text_set.add(
                    (self.tokenizer.decode(input_ids[h_e]), pred, self.tokenizer.decode(input_ids[t_e]))
                )
                spo_text_set.add(
                    (
